import os, io, html, datetime as dt
import pandas as pd
import yfinance as yf

OUT_DIR = "reports"
ARCHIVE = True  # save a dated copy too

def fetch_sparkline_series(ticker: str, days: int = 45):
    end = dt.date.today()
    start = end - dt.timedelta(days=days*2)  # extra buffer for weekends/holidays
...
def to_svg_sparkline(series, width=120, height=28, pad=4):
    if not series:
        return ""
...
def conf_bar(conf: float):
    pct = max(0,min(99,int(round(conf*100))))
    return f"""
<div class="bar">
  <div class="fill" style="width:{pct}%;"></div>
  <span class="pct">{conf:.2f}</span>
</div>
""".strip()
...
def main():
    ...
    if not os.path.exists(csv_path):
        raise SystemExit("Missing data/signals_latest.csv")

    df = pd.read_csv(csv_path)
    if df.empty:
        raise SystemExit("signals_latest.csv is empty")

    # Rank SELL first, then BUY by confidence desc, then HOLD
    ...
    for t in unique_tickers:
        try:
            s = fetch_sparkline_series(t, days=30)
        except Exception:
            s = []
        series_map[t] = to_svg_sparkline(s)
    ...
    for _, r in df.iterrows():
        ...
        spark = series_map.get(r.ticker, "")
        reasons = html.escape(str(r.reasons))
        features = html.escape(str(r.features_json))
        rows_html.append(f"""
<tr class="{klass}">
  <td class="sticky">{html.escape(r.ticker)}</td>
  ...
  <td class="left small">{features}</td>
</tr>""")
...
<div class="wrap">
  ...
  <div class="legend">SELL rows appear first, then BUY (by confidence), then HOLD. Sparklines show the last ~30 trading days (auto-adjusted close).</div>
    ...
    # Write index + (optional) archive copy
    index_path = os.path.join(OUT_DIR, "index.html")
    open(index_path, "w").write(html_doc)
    if ARCHIVE:
        d = dt.date.today().isoformat()
        open(os.path.join(OUT_DIR, f"{d}.html"), "w").write(html_doc)
